...
class SkillsRegistry:
    ...
    def from_tool_manager(cls, node, timeout_sec: float = 10.0) -> "SkillsRegistry":
        ...
        try:
            from andr_msgs.srv import ListTools  # noqa: PLC0415
        except ImportError:
        ...
        # Use spin_until_future_complete so the response is processed even
        # when called during __init__ (before the main executor is spinning).
        _rclpy.spin_until_future_complete(node, future, timeout_sec=timeout_sec)

        if not future.done() or future.result() is None:
            logger.warning("tool_manager/list timed out or failed — using empty registry.")
            return cls()
    ...
    def refresh(self, timeout_sec: float = 5.0) -> list[str]:
        ...
        import json as _json
        try:
            from andr_msgs.srv import ListTools
        except ImportError:
            return []
...
class SkillExecutor:
    ...
    def __init__(
        ...
        try:
            ...
            from andr_msgs.action import ExecuteSkill           # noqa: PLC0415
