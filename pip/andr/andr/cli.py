...
def cmd_task(args):
    ...
    goal.priority = max(1, min(10, args.priority))
...
def cmd_add(args):
    """Scaffold a new tool, input, or runnable in the current project."""
    kind = args.kind
    name = args.name

    # Determine target directory
    dir_map = {"tool": "tools", "input": "inputs", "runnable": "runnables"}
    target_dir = os.path.join(os.getcwd(), dir_map[kind])

    if not os.path.isdir(target_dir):
        print(f"Error: '{dir_map[kind]}/' directory not found in current directory.")
        print("Are you inside an ANDR project?  (Run 'andr init <name>' to create one.)")
        sys.exit(1)

    # Build filename from name (snake_case)
    filename = name.replace("-", "_").lower() + ".py"
    filepath = os.path.join(target_dir, filename)

    if os.path.exists(filepath):
        print(f"Error: {dir_map[kind]}/{filename} already exists.")
        sys.exit(1)
    ...
    if kind == "tool":
        content = _scaffold_tool(name, class_name, memory=args.memory)
    elif kind == "input":
        content = _scaffold_input(name, class_name)
    elif kind == "runnable":
        content = _scaffold_runnable(name, class_name)

    _write(os.getcwd(), os.path.join(dir_map[kind], filename), content)
    print(f"Created {dir_map[kind]}/{filename}")


def _scaffold_tool(name, class_name, memory=False):
    """Generate a BaseAgentTool scaffold."""
    ...
    if memory:
        ...
        memory_execute = (
            ...
            "        # memories = self._memory.query(\"your query\", top_k=4)\n"
            "\n"
            "        # Store a new memory\n"
            "        # self._memory.add(\"something worth remembering\", metadata={\"source\": self.TOOL_NAME})\n"
        )

    return f'''"""{class_name} tool — TODO: describe what this tool does.

Auto-generated by `andr add tool {name}`.
"""
...
class {tool_class}(BaseAgentTool):
    TOOL_NAME = "{snake}"
    TOOL_DESCRIPTION = "TODO: describe what this tool does"
    TOOL_PARAMETERS = [
    ...
    @dataclass
    class ParamsType:
        pass  # TODO: add typed parameters

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        # TODO: set up publishers, subscribers, clients, etc.
{memory_init}
    def _execute(self, params: ParamsType, goal_handle) -> dict:
        self.get_logger().info(f"[{snake.upper()}] Executing...")
{memory_execute}
...
def _scaffold_input(name, class_name):
    """Generate a BaseInputSource scaffold."""
    snake = name.replace("-", "_").lower()
    input_class = f"{class_name}Input"

    return f'''"""{class_name} input source — TODO: describe what triggers tasks.

Auto-generated by `andr add input {name}`.
"""
...
class {input_class}(BaseInputSource):
    SOURCE_NAME = "{snake}"
    SOURCE_DESCRIPTION = "TODO: describe what this input source does"

    def __init__(self):
        super().__init__()
        # TODO: subscribe to topics or set up listeners
        # self.create_subscription(String, "/my_topic", self._on_message, 10)

    # def _on_message(self, msg):
    #     self.send_task(
    #         prompt=f"Received: {{msg.data}}",
    #         context="{snake}_message",
    #         priority=5,  # 1–10, higher = more important (default 5)
    #     )

    # ── Task lifecycle hooks (override as needed) ────────────────────

    def on_task_accepted(self, prompt: str) -> None:
        self.get_logger().info(f"[{snake}] Task accepted")

    def on_task_rejected(self, prompt: str) -> None:
        self.get_logger().warn(f"[{snake}] Task rejected")

    def on_task_completed(self, prompt: str, success: bool, summary: str) -> None:
        self.get_logger().info(f"[{snake}] Task completed: {{summary}}")
...
def _scaffold_runnable(name, class_name):
    """Generate a standalone ROS 2 node scaffold."""
    snake = name.replace("-", "_").lower()
    node_class = f"{class_name}Node"

    return f'''"""{class_name} runnable — TODO: describe what this node does.

Standalone ROS 2 node auto-discovered and launched by start.py.
Auto-generated by `andr add runnable {name}`.
"""
...
class {node_class}(Node):
    def __init__(self):
        super().__init__("{snake}_node")
        self.get_logger().info("{node_class} started")

        # TODO: set up publishers, subscribers, timers, services, etc.
...
Commands:
  ...
  add       Scaffold a new tool, input, or runnable
...
""",
    ...
    p_add = sub.add_parser(
        "add",
        help="Scaffold a new tool, input, or runnable",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Generates a starter file in the current ANDR project directory.

Types:
  tool       BaseAgentTool — a capability the agent can call
  input      BaseInputSource — sends tasks to the agent pipeline
  runnable   Standalone ROS 2 node (auto-discovered by start.py)

Options:
  --memory   (tool only) Include ChromaDB memory store boilerplate

Examples:
  andr add tool gripper
  andr add tool remember --memory
  andr add input voice
  andr add runnable battery_monitor
""",
    )
    p_add.add_argument("kind", choices=["tool", "input", "runnable"],
                       help="What to scaffold: tool, input, or runnable")
    p_add.add_argument("name", help="Name for the new component (snake_case)")
    p_add.add_argument("--memory", action="store_true", default=False,
                       help="(tool only) Include memory store boilerplate")
...
Priority (1–10, default 5):
  ...
  Preempted tasks are queued and resumed after the interrupting task completes.

Examples:
  andr task "Say hello"
  andr task "Walk forward 2 meters"
  andr task "Navigate to the kitchen" --context "map:home"
  andr task "Emergency stop" --priority 10
...
""",
    ...
    p_task.add_argument("--priority", type=int, default=5,
                        help="Task priority 1–10 (higher = more important, default: 5)")
